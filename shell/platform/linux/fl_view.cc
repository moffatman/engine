...
static void event_box_gesture_rotation_begin(GtkGestureRotate* gesture,
                                             GdkEventSequence* sequence,
                                             FlView* view);

static void event_box_gesture_rotation_update(GtkGestureRotate* widget,
                                              gdouble angle,
                                              gdouble delta,
                                              FlView* view);
...
static void event_box_gesture_zoom_begin(GtkGestureZoom* gesture,
                                         GdkEventSequence* sequence,
                                         FlView* view);
...
static void fl_view_constructed(GObject* object) {
  ...
  g_signal_connect(zoom, "begin", G_CALLBACK(event_box_gesture_zoom_begin),
                   self);
  g_signal_connect(zoom, "scale-changed",
                   G_CALLBACK(event_box_gesture_zoom_update), self);
  g_signal_connect(zoom, "end", G_CALLBACK(event_box_gesture_zoom_end), self);
  GtkGesture* rotate = gtk_gesture_rotate_new(self->event_box);
  g_signal_connect(rotate, "begin",
                   G_CALLBACK(event_box_gesture_rotation_begin), self);
  g_signal_connect(rotate, "angle-changed",
                   G_CALLBACK(event_box_gesture_rotation_update), self);
  g_signal_connect(rotate, "end", G_CALLBACK(event_box_gesture_rotation_end),
                   self);
}
...
static gboolean event_box_scroll_event(GtkWidget* widget,
  ...
  if (gdk_device_get_source(gdk_event_get_source_device((GdkEvent*)event)) ==
    ...
    scroll_delta_y *= -1;
    if (event->is_stop) {
      fl_engine_send_pointer_flow_event(
          view->engine, 123123, event->time * kMicrosecondsPerMillisecond,
          event->x * scale_factor, event->y * scale_factor, kFlowEnd,
      ...
      if (!view->pan_started) {
        ...
        fl_engine_send_pointer_flow_event(
            view->engine, 123123, event->time * kMicrosecondsPerMillisecond,
            event->x * scale_factor, event->y * scale_factor, kFlowStart, 0, 0,
            0, 0);
      ...
      fl_engine_send_pointer_flow_event(
          view->engine, 123123, event->time * kMicrosecondsPerMillisecond,
          event->x * scale_factor, event->y * scale_factor, kFlowUpdate,
...
static void check_pointer_inside(FlView* view, GdkEvent* event) {
  if (!view->pointer_inside) {
    ...
    if (gdk_event_get_coords(event, &x, &y)) {
      gint scale_factor = gtk_widget_get_scale_factor(GTK_WIDGET(view));
      view->last_x = x * scale_factor;
      view->last_y = y * scale_factor;
      fl_engine_send_mouse_pointer_event(
          view->engine, kAdd,
...
static void event_box_gesture_rotation_begin(GtkGestureRotate* gesture,
  ...
  if (!view->zoom_rotate_started) {
    ...
    view->scale = 1;
    view->angle = 0;
    fl_engine_send_pointer_flow_event(view->engine, 123123, g_get_real_time(),
                                      view->last_x, view->last_y, kFlowStart, 0,
                                      0, 0, 0);
  }
}
...
static void event_box_gesture_rotation_update(GtkGestureRotate* widget,
  ...
  view->angle = angle;
  fl_engine_send_pointer_flow_event(view->engine, 123123, g_get_real_time(),
                                    view->last_x, view->last_y, kFlowUpdate, 0,
                                    0, view->scale, view->angle);
}
...
static void event_box_gesture_rotation_end(GtkGestureRotate* gesture,
  ...
  if (view->zoom_rotate_started) {
    ...
    fl_engine_send_pointer_flow_event(view->engine, 123123, g_get_real_time(),
                                      view->last_x, view->last_y, kFlowEnd, 0,
                                      0, 0, 0);
  }
}
