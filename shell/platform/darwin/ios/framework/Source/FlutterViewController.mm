...
static flutter::PointerData::Change PointerDataChangeFromUIGestureRecognizerState(UIGestureRecognizerState state) {
  switch (state) {
    ...
    case UIGestureRecognizerStateCancelled:
      return flutter::PointerData::Change::kCancel;
    default:
      FML_DLOG(INFO) << "Unhandled gesture state: " << state;
      break;
  }
...
- (void)panEvent:(UIPanGestureRecognizer*)recognizer API_AVAILABLE(ios(13.4)) {
  ...
  pointer_data.change = PointerDataChangeFromUIGestureRecognizerState(recognizer.state);

  pointer_data.kind = flutter::PointerData::DeviceKind::kTouch;
  pointer_data.device = reinterpret_cast<int64_t>(recognizer);

  pointer_data.physical_x = translation.x * scale + _mouseState.location.x;
  pointer_data.physical_y = translation.y * scale + _mouseState.location.y;
