...
- (void)testPointerHover {
  ...
  // Ideally we would also test whether the hover pointer is re-added after a tap
  // There seems to be a bug in XCTest/Xcode, after a tap event, the hover selector doesn't work any
  // more
  for (XCUIElement* textField in app.textFields.allElementsBoundByIndex) {
